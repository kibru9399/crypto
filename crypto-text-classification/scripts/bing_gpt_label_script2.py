...
async def generate_labels(data, cookies, save_rate=5, start_index=0, max_count=300):
    ...
    try:
        for i, text in tqdm(data['text'].items(), total=min(max_count, len(data))):
            ...
            data.loc[i, 'label_text'] = response['text']
            #data['label_sources_text'].iloc[i] = response['sources_text']
            data.loc[i, 'label_sources_text'] = response['sources_text']
            if i % save_rate == 0:
                logger.info('Dumping Data...')
                data.to_csv(r'C:\Users\Kebab\Documents\datamining\crypto-text-classification\output\classification\crypto_news_plus.csv', index=False)
